...
Utils.C_EPSILON = 1.192092896e-7; // Math.pow(2, -23) float
Utils.C_EPSILON1000 = Utils.C_EPSILON * 1000;
...
Utils.prototype.loadStylesheet = function (href, opt) {
	opt = opt || {};
...
Utils.prototype.prepareUrl = function (url) {
	return url;
};
...
Utils.prototype.extend = (function () {
	var extendStatics = Object.setPrototypeOf ||
		...
		function (Child, Parent) { for (var p in Parent) if (Object.prototype.hasOwnProperty.call(Parent, p)) Child[p] = Parent[p]; };
...
Utils.prototype.reExtend = function (Child, Parent, ChildNew) {
...
Utils.prototype.mix = function (Child, Parent, opt) {
	opt = opt || {};
...
Utils.prototype.delegate = function (context, methodName, constructor, args) {
...
Utils.prototype.overrideMethod = function (Class, methodName, method, Childs) {
...
Utils.prototype.reOverrideMethod = function (Class, methodName, method) {
...
Utils.prototype.overrideFunc = function (obj, funcName, func) {
	func.origin = obj[funcName];

	return obj[funcName] = func;
};

Utils.prototype.reOverrideFunc = function (obj, funcName, func) {
...
Utils.prototype.runFunc = function (func) {
	return func();
};

Utils.prototype.oddFunc = function (arr, level) {
	if (!arr)
		return 0;
...
Utils.prototype.oddFuncReverse = function (arr, res) {
	if (!arr)
		return 0;
...
Utils.prototype.getAdmSuffix = function () {
...
Utils.prototype.isArray = function (foo) {
...
Utils.prototype.removeFromArray = function (arr, val) {
	arr = arr || [];
...
Utils.prototype.inStr = function (str, val) {
	return (str || []).indexOf(val) != -1
};

Utils.prototype.clone = function (obj, own) {
...
Utils.prototype.cloneBase = function (obj) {
...
Utils.prototype.isEqual = function (obj1, obj2, compare_func) {
...
Utils.prototype.isArrsIntersects = function (arr1, arr2) {
...
Utils.prototype.getPosFromEvent = function (e, prop) {
	prop = prop || 'page';

	return { x: e[prop + 'X'], y: e[prop + 'Y'] };
};

Utils.prototype.getMouseButton = function (e) {
	// Для ie
	if (!e.which && e.button) {
...
Utils.prototype.mobileClick = function (el, opt) {
	opt = opt || {};
...
Utils.prototype.cloneFS = function (obj, stack) {
	if (!stack) stack = [];
...
Utils.prototype.copy = function (to, from) {
	return $.extend(to, from);
};

Utils.prototype.copyProperties = function (to, from, opt) {
	opt = opt || {};
	to = to || {};
...
Utils.prototype.cloneInstance = function (instance, opt) {
	opt = opt || {};
	instance = instance || {};
...
Utils.prototype.isInstance = function (obj) {
	if (obj == null)
		return false;
...
Utils.prototype.decodeURIComponent = function (str) {
	var out = '', arr, i = 0, l, x;
...
Utils.prototype.charToInt = function (l) {
	if (!l) return -1;
	var code = 0;
...
Utils.prototype.intToChar = function (numb, count) {
	count = count || 1;
...
Utils.prototype.bitmapCount = function (bitmap) {
	var count = 0;
	var bit = 1;
	while (bitmap >= bit) {
		if (bitmap & bit) {
...
Utils.prototype.bin2dec = function (bin, rev) {
	var dec = 0;
...
Utils.prototype.dec2bin = function (dec, isReverse, fillTo) {
...
Utils.prototype.dec2arr = function (dec) {
	var arr = {};
	var i = 0;
	while (dec != 0) {
...
Utils.prototype.calcColor = function (str) {
...
Utils.prototype.clearString = function (str) {
	return str.split('').join('');
};

Utils.prototype.twoDigits = function (i) {
	if (typeof (i) == 'string') {
		while (i.length < 2) {
			i = '0' + i;
		}
...
Utils.prototype.prepZero = function (i, n) {
	i = '' + i;
	while (i.length < n) {
		i = '0' + i;
	}
...
Utils.prototype.upFirstLetter = function (text) {
	return text.charAt(0).toUpperCase() + text.slice(1);
};

Utils.prototype.downFirstLetter = function (text) {
	return text.charAt(0).toLowerCase() + text.slice(1);
};

Utils.prototype.KMFloor = function (num, rank, stages) {
...
Utils.prototype.prepCount = function (count) {
	count = count || 0;
	var sign = utils.selectKMSign(count);
	return sign ? utils.KMFloor(count) : utils.stages(count);
};

Utils.prototype.selectKMSign = function (num, minK, minM) {
	num = Math.abs(num);
...
Utils.prototype.toFixed = function (val, dec, simpleFixed) {
...
Utils.prototype.toInt = function (val) {
	if (Math.abs(val) > 2e9)
...
Utils.prototype.signFixed = function (num, fix, simpleFixed) {
	return (num > 0 ? '+' : '') + utils.toFixed(num, fix, simpleFixed);
};

Utils.prototype.formatNum = (function () {
...
Utils.prototype.toPercent = function (val, dec) {
	val *= 100.001;

	return dec ? utils.toFixed(val, dec) : utils.toInt(val);
};

Utils.prototype.fixDecimal = function (val) {
	return Math.round(val * 100) / 100;
};

Utils.prototype.stages = function (num) {
...
Utils.prototype.getDec = function (num) {
	return ('' + num).replace(/[0-9,-]+\d*/, '');
};

Utils.prototype.hexToRGB_A = function (hex, noAlphaNormalize) {
...
Utils.prototype.calcGradByPercent = function (percent, palette) {
	palette = palette || [
...
Utils.prototype.unescapeHtml = function (str) {
	return (str || '')
		.replace(/&amp;/g, '&')
		.replace(/&lt;/g, '<')
		.replace(/&gt;/g, '>')
...
Utils.prototype.escapeHtml = function (str) {
	return str
		.replace(/[&]/g, '&amp;')
		.replace(/[<]/g, '&lt;')
		.replace(/[>]/g, '&gt;')
...
Utils.prototype.escapeHtmlChat = function (str) {
	return str
		.replace(/[&]/g, '&amp;')
		.replace(/[<]/g, '&lt;')
		.replace(/[>]/g, '&gt;')
...
Utils.prototype.escapeHtmlChatMess = function (str) {
	return str
		.replace(/[&]/g, '&amp;')
		.replace(/[<]/g, '&lt;')
		.replace(/[>]/g, '&gt;')
...
Utils.prototype.cnstName = function (cnstBlock, val) {
...
Utils.prototype.createNames = function (from, to) {
	for (var name in from) {
		to[from[name]] = name;
	}
};

Utils.prototype.createIds = function (from, to, field) {
	if (!field) field = 'name';
	for (var id in from) {
		to[from[id][field]] = +id;
	}
...
Utils.prototype.htmlToText = function (html, opt) {
	if (!html)
		return '';
...
Utils.prototype.makeAttrStr = function (attrs, prefix) {
	if (!attrs) return '';
...
Utils.prototype.makeDataAttrStr = function (attrs) {
	return utils.makeAttrStr(attrs, 'data-');
};

Utils.prototype.parseResString = function (str) {
	var result = [];
...
Utils.prototype.parseString = function (str, char) {
	var result = [];
...
Utils.prototype.splitPosArr = function (arr) {
	return {
		o: arr[0],
		x: arr[1],
		y: arr[2]
...
Utils.prototype.splitTownArr = function (a, b) {
	if (typeof (a) == 'object') {
		return {
			id: a[0],
			name: a[1],
...
Utils.prototype.splitAccountArr = function (a, b) {
...
Utils.prototype.clearForm = function ($form, opt) {
	opt = opt || {};
...
Utils.prototype.toggleAttr = function (elem, attr, val) {
	if (val === 'undefined')
...
Utils.prototype.assignLengthCounter = function (textElem, max, counter) {
...
Utils.prototype.textSelection = function (el) {
	el = $(el).get(0);
...
Utils.prototype.isEclipsed = function (el) {
	return el.scrollWidth > el.clientWidth;
};

Utils.prototype.urlToObj = function (url, noLocation) {
	var params = {};
	...
	for (var i in arr) {
		if (arr[i].indexOf('=') == -1) {
			...
			params[part.shift()] = decodeURIComponent(part.join('=').replace(/[+]/g, ' '));//правим все плюсы
		}
	}
	return params;
};

Utils.prototype.urlToObj2 = function (url) {
...
Utils.prototype.serializeToObject = function (str, checkSame) {
	var params = {};
...
Utils.prototype.getHashParams = function (param) {
	var hashObj = utils.urlToObj(location.search);
	return param ? hashObj[param] : hashObj;
};

Utils.prototype.objToUrl = function (obj) {
	var url = '';
...
Utils.prototype.objToUrl2 = function (obj) {
	var url = obj.path;
...
Utils.prototype.hasMask = function (mask, baseMask) {
	return ((mask || 0) & (baseMask || 0)) > 0;
};

Utils.prototype.inMask = function (ind, mask) {
	return ((1 << ind) & (mask || 0)) > 0;
};

Utils.prototype.toggleMask = function (ind, mask) {
	return mask ^ (1 << ind);
};

Utils.prototype.offMask = function (ind, mask) {
...
Utils.prototype.onMask = function (ind, mask) {
	return mask | (1 << ind);
};

Utils.prototype.isTrue = function (variable) {
	return variable === true;
};

Utils.prototype.isFalse = function (variable) {
	return variable === false;
};

Utils.prototype.isNull = function (variable) {
	return variable === null;
};

Utils.prototype.isUndf = function (variable) {
	return variable === undefined;
};

Utils.prototype.random = function (n) {
	var rnd = Math.random();
	if (rnd == 1) rnd = 0;
	return ~~(rnd * n);
};

Utils.prototype.randomSeed = function (n) {
	return ~~(invwk.rand() * n);
};

Utils.prototype.colorGoogle = function (str) {
	var blue = '#4285F4';
	var red = '#EA4335';
	var yellow = '#FBBC05';
	var green = '#34A853'
...
Utils.prototype.log10 = function (int) {
	if (Math.log10) {
...
Utils.prototype.nullToArr = function (smth) {
	return smth == null ? [] : smth;
};

Utils.prototype.toArr = function (val) {
	return utils.isArray(val) ? val : [val];
};

Utils.prototype.objToArr = function (obj, sortFunc) {
	var arr = [];
	for (var i in obj) {
		arr.push(obj[i]);
	}
...
Utils.prototype.arrToObj = function (arr, toKeys, val) {
	var obj = {};
	if (toKeys) {
		for (var i in arr) {
...
Utils.prototype.calcObjSum = function (obj) {
	var sum = 0;
	for (var i in obj) {
		sum += obj[i];
	}
...
Utils.prototype.noPx = function (val) {
	if (!val) return 0;
	return +val.slice(0, val.length - 2)
};

Utils.prototype.nobr = function (text) {
	return text.replace(/<br>/g, ' ');
};

Utils.prototype.getStrByteLength = function (str) {
...
Utils.prototype.getSeedByStr = function (str) {
...
Utils.prototype.getElemSize = function ($cont, opt) {
	opt = opt || {};
	...
	}).call(opt.context, $cont);

	if (opt.callback)
		opt.callback.call(opt.context, $cont, size);
...
Utils.prototype.inDOM = (function () {
	if (typeof (document) != 'object')
		return function () { return false; }; // В воркере нет объекта document

	var doc = document.contains ? document : document.documentElement;
...
Utils.prototype.adjustBlocksHeight = function ($wrp, cls, callback) {
	var groups = [],
		groupI = -1,
		oldOffset = -1;
...
Utils.prototype.adjustBlocksWidth = function ($wrp, cls, callback) {
	var groups = [],
		groupI = -1,
		oldOffset = -1;
...
Utils.prototype.animateElem = (function () {
	if (typeof (document) != 'object')
...
Utils.prototype.runTransition = (function () {
	if (typeof (document) != 'object')
		return function () { }; // В воркере нет объекта document

	var params,
		setCallback = (function () {
			var computedStyles, mult;

			return function (el, callback, duration) {
				clearTimeout(el._transitionCallbackTO_);
				...
				if (duration === undefined) {
					...
					duration = computedStyles.getPropertyValue('transition-duration') || computedStyles.getPropertyValue('-webkit-transition-duration') || '0ms';
					...
					duration = parseFloat(duration) * mult;
				}

				el._transitionCallbackTO_ = setTimeout(callback.bind(el), duration);
			};

			/*
			Много условий при которых событие transitionend не вызывается. К примеру если transition-duration: 0, значение свойств не изменилось (переход небыл запущен) или блок находится под display: none
			var eventHandlerProp;

			if( document.documentElement.ontransitionend !== undefined )
				eventHandlerProp = 'transitionend';
			else if( document.documentElement.onwebkittransitionend !== undefined )
				eventHandlerProp = 'webkitTransitionEnd';

			if( !eventHandlerProp ){
				return function(el, callback, duration){
					clearTimeout(el._transitionTO_);

					if( !callback )
						return;

					if( duration === undefined ){
						var styles = window.getComputedStyle(el), mult = 1;

						duration = styles.getPropertyValue('transition-duration')||styles.getPropertyValue('-webkit-transition-duration')||'0ms';
					...
					el._transitionTO_ = setTimeout(callback.bind(el), duration);
				};
			}
			
			var removeCallback = function(el){ // "once" prop is not supported by old browsers
				if( !el._transitionCallback_ )
					return;
				
				el.removeEventListener(eventHandlerProp, el._transitionCallback_, false);
				
				delete el._transitionCallback_;
			};
			...
			return function(el, callback){
				...
				el._transitionCallback_ = function(){
					removeCallback(this);
					
					callback.call(this);
				};
				
				el.addEventListener(eventHandlerProp, el._transitionCallback_, false);
			};
			*/
		})(),
	...
	return function (el, opt) {
		...
		setCallback(el, opt.callback, params.duration);

		setDataAttrs(el, opt.dataset);
	};
})();

Utils.prototype.triggerTransition = function (el) {
	if (!el)
		return;

	if (el instanceof jQuery) {
		el.each(function () {
			utils.triggerTransition(this);
		});

		return;
	}

	el.scrollWidth++; // Trigger rendering
...
Utils.prototype.getWindowWidth = function (val, minVal) {
...
Utils.prototype.getDocWidth = function (val, minVal) {
	minVal = minVal === undefined ? 550 : minVal;
...
Utils.prototype.isDocVisible = function () {
	return document.visibilityState != 'hidden';
};

Utils.prototype.isNodeUserUnselectable = (function () {
	if (typeof (document) !== 'object')
		return function () { return false; };

	var rangeWas, unselectable, selectionDef, nodeForSelection = document.createElement('span');

	if (document.getSelection)
		selectionDef = document.getSelection();
	else if (window.getSelection)
		selectionDef = window.getSelection();

	nodeForSelection.innerText = '-';

	nodeForSelection.style.position = 'fixed';
	nodeForSelection.style.opacity = '0';
	nodeForSelection.style.zIndex = '-100';

	return function (node, selection) {
		if (!node)
			return false;

		selection = selection || selectionDef;
		...
		if (selection.rangeCount)
			rangeWas = selection.getRangeAt(0);

		node.appendChild(nodeForSelection);

		selection.selectAllChildren(nodeForSelection);
		unselectable = !selection.toString();

		nodeForSelection.remove();

		if (rangeWas) {
			selection.removeAllRanges();

			selection.addRange(rangeWas);
		}
...
Utils.prototype.objMath = function (obj1, action, obj2) {
...
Utils.prototype.chClass = function (el, prefix, val) {
...
Utils.prototype.genName = function (sex, name, opt) {
	opt = opt || {};
...
Utils.prototype.isObjectsEqual = function (obj1, obj2) {
...
Utils.prototype.getFullScreenDoc = function () {
	try {
...
Utils.prototype.isFullScreenAvail = function () {
...
Utils.prototype.ifUndf = function (val, undfVal) {
	return typeof (val) == 'undefined' ? undfVal : val;
};

Utils.prototype.checkClick = function (event, selector) {
	return $(event.target).is(selector) || $(event.target).parents(selector).length;
};

Utils.prototype.isTouchDevice = function () {
...
Utils.prototype.checkLimits = function (name, limits) {
...
Utils.prototype.checkInputInt = function (el, opt) {
	opt = opt || {};
...
Utils.prototype.checkInputInt.checkMin = function($el, val, opt){
	var valStr = '' + val,
		minValStr = '' + opt.min,
...
Utils.prototype.checkInputFloat = function (el, maxVal, minVal, fixed) {
	var $el = $(el),
		val = $el.val(),
		sign = val[0] == '-' ? '-' : '',
...
Utils.prototype.getInputTextRange = function (el) {
	if (el instanceof jQuery)
		el = el.get(0);
...
Utils.prototype.setInputTextRange = function (el, textRange) {
	var $el = el;
...
module.exports = { Utils, utils, invwk, runOnce, isLSAvail, isSSAvail };