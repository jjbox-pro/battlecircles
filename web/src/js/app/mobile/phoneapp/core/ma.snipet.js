...
snip.fixedHint = (function(){
    var optText, optHint, prop, extend = function(to, from){
        for(prop in from)
            to[prop] = from[prop];
    ...
    return function(num, opt, optInit, optExt){
        optText = {};
            
        if( optInit && optInit[0] )
            extend(optText, optInit[0]);
        else{
            optText.toNum = 
            optText.mixed = true;
            optText.uFixed = 1;
        ...
        if( optInit && optInit[1] )
            extend(optHint, optInit[1]);
        else{
            optHint.toNum = 
            optHint.mixed = 
            optHint.fixStages = true;
            optHint.uFixed = 3;
        }
        ...
        if( optExt ){
            if( optExt[0] ) extend(optText, optExt[0]);
            if( optExt[1] ) extend(optHint, optExt[1]);
        }

        return snip.orientationCont((optText.formatFunc||utils.formatNum)(num, optText), (optHint.formatFunc||utils.formatNum)(num, optHint));
    };
})();